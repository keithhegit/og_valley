...
基于 PRD 与当前实现的对比，以下功能尚未实现：

### 🔴 高优先级缺口
1. **时间系统** (昼夜循环) - PRD 要求但完全未实现
2. **体力消耗强制** - 部分实现但缺少"耗尽无法工作"的限制
3. **玩家死亡/惩罚** - 未实现
4. **食物消耗系统** - 未实现
5. **存档系统** - 未实现
...
8. **音效系统** - 未实现
9. **季节系统** - 数据结构存在但未使用
10. **House 室内场景** - 仅有外观，无法进入

### 🟢 低优先级缺口
11. **鼠标点击交互** - 仅支持键盘
12. **镰刀 (Scythe)** - 未添加
13. **钓竿 (Rod)** - 未添加
14. **特殊矿脉 Nodes** - 仅有普通石头
15. **NPC 好感度系统** - 仅有简单对话
...
### 🚧 第五阶段：打磨与持久化 (当前)
-   [ ] **时间系统**: 昼夜循环，每 tick 10 分钟，2:00 AM 强制睡觉。
-   [ ] **体力管理**: 强制体力消耗，耗尽无法使用工具。
-   [ ] **食物系统**: 添加可食用物品，回复体力。
-   [ ] **玩家死亡**: HP 归零时昏迷，丢失金钱并传送回家。
-   [ ] **存档系统**: `localStorage` 持久化 (未来迁移至 Workers KV)。
-   [ ] **视觉效果**: 工具挥动动画、玩家行走帧。
-   [ ] **音频**: Web Audio API 音效与背景音乐。
